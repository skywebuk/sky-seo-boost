            ...
            if (!empty($post_ids)) {
                switch ($action) {
                    ...
                    case 'trash':
                        ...
                        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Selected posts moved to trash.', 'sky360') . '</p></div>';
                        break;
                    case 'untrash':
                        foreach ($post_ids as $post_id) {
                            if (current_user_can('delete_post', $post_id)) {
                        ...
                        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Selected posts restored from trash.', 'sky360') . '</p></div>';
                        break;
                    case 'delete':
                        foreach ($post_ids as $post_id) {
                            if (current_user_can('delete_post', $post_id)) {
                        ...
                        echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Selected posts permanently deleted.', 'sky360') . '</p></div>';
                        break;
                    case 'duplicate':
                        ...
                        if ($duplicated > 0) {
                            echo '<div class="notice notice-success is-dismissible"><p>' . sprintf(_n('%s item duplicated successfully.', '%s items duplicated successfully.', $duplicated, 'sky360'), $duplicated) . '</p></div>';
                        }
                        break;
                }
            }
                ...
                <form method="get" class="sky-seo-filters-form">
                    ...
                    <div class="sky-seo-filter-row">
                        <div class="sky-seo-filter-group">
                            <label for="s"><?php _e('Search', 'sky360'); ?></label>
                            <input type="text" name="s" id="s" placeholder="<?php _e('Search posts...', 'sky360'); ?>" value="<?php echo esc_attr($search_term); ?>">
                        </div>
                        ...
                        <div class="sky-seo-filter-actions">
                            ...
                            <a href="<?php echo admin_url('edit.php?post_type=' . $typenow . ($post_status === 'trash' ? '&post_status=trash' : '')); ?>" class="button button-link"><?php _e('Clear', 'sky360'); ?></a>
                        </div>
                    </div>
                </form>
            </div>
                    ...
                    <div class="alignleft actions">
                        <select name="action" class="sky-seo-bulk-select">
                            ...
                            <?php if ($post_status === 'trash') : ?>
                                <option value="untrash"><?php _e('Restore', 'sky360'); ?></option>
                                <option value="delete"><?php _e('Delete Permanently', 'sky360'); ?></option>
                            <?php else : ?>
                                <option value="publish"><?php _e('Publish', 'sky360'); ?></option>
                                <option value="draft"><?php _e('Set to Draft', 'sky360'); ?></option>
                                <option value="trash"><?php _e('Move to Trash', 'sky360'); ?></option>
                                <?php if ($duplicate_enabled) : ?>
                                    <option value="duplicate"><?php _e('Duplicate', 'sky360'); ?></option>
                                <?php endif; ?>
                            <?php endif; ?>
                        </select>
                    ...
                    <table class="sky-seo-content-table">
                        <thead>
                            <tr>
                                ...
                                <th class="column-author"><?php _e('Author', 'sky360'); ?></th>
                                <th class="column-clicks">
                                    <span class="dashicons dashicons-chart-bar"></span>
                                    <?php _e('Total Clicks', 'sky360'); ?>
                                </th>
                                <th class="column-traffic"><?php _e('Traffic Sources', 'sky360'); ?></th>
                                <th class="column-status"><?php _e('Status', 'sky360'); ?></th>
                                <th class="column-date"><?php _e('Date', 'sky360'); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr class="level-<?php echo $level; ?>">
                                    ...
                                    <td class="column-title">
                                        ...
                                        <div class="row-actions">
                                            <?php if ($post_status === 'trash') : ?>
                                                <span class="untrash">
                                                    <a href="<?php echo wp_nonce_url(admin_url(sprintf('post.php?post=%d&action=untrash', $post_id)), 'untrash-post_' . $post_id); ?>"><?php _e('Restore', 'sky360'); ?></a> | 
                                                </span>
                                                <span class="delete">
                                                    <a href="<?php echo get_delete_post_link($post_id, '', true); ?>" class="submitdelete"><?php _e('Delete Permanently', 'sky360'); ?></a>
                                                </span>
                                            <?php else : ?>
                                                <span class="edit">
                                                    <a href="<?php echo get_edit_post_link($post_id); ?>"><?php _e('Edit', 'sky360'); ?></a> | 
                                                </span>
                                                <?php 
                                                ...
                                                if (defined('ELEMENTOR_VERSION') && \Elementor\Plugin::$instance->documents->get($post_id)) : ?>
                                                    <span class="edit-with-elementor">
                                                        <a href="<?php echo esc_url(\Elementor\Plugin::$instance->documents->get($post_id)->get_edit_url()); ?>" target="_blank"><?php _e('Edit with Elementor', 'sky360'); ?></a> | 
                                                    </span>
                                                <?php endif; ?>
                                                <span class="view">
                                                    <a href="<?php echo get_permalink($post_id); ?>" target="_blank"><?php _e('View', 'sky360'); ?></a> | 
                                                </span>
                                                <span class="trash">
                                                    <a href="<?php echo get_delete_post_link($post_id); ?>" class="submitdelete"><?php _e('Trash', 'sky360'); ?></a>
                                                </span>
                                                <?php if ($duplicate_enabled) : ?>
                                                    <?php
                                                    ...
                                                    <span class="duplicate"> | 
                                                        <a href="<?php echo esc_url($duplicate_url); ?>"><?php _e('Duplicate This', 'sky360'); ?></a>
                                                    </span>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <td class="column-status">
                                        ...
                                        $status_labels = [
                                            ...
                                            'trash' => ['label' => __('Trash', 'sky360'), 'class' => 'trash']
                                        ];
                                ...
                                <tr>
                                    <td colspan="7" class="no-items">
                                        <div class="sky-seo-no-content">
                                            ...
                                            <p><?php _e('No posts found.', 'sky360'); ?></p>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
