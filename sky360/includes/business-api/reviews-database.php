...
class Sky_SEO_Reviews_Database {
    ...
    public function add_cron_schedules($schedules) {
        $schedules['every_48_hours'] = [
            ...
            'display' => __('Every 48 hours', 'sky360')
        ];
        return $schedules;
    }
    ...
    public function ajax_save_manual_review() {
        ...
        if (!current_user_can('manage_options')) {
            wp_send_json_error(__('Permission denied', 'sky360'));
        }
        ...
        if (empty($author_name)) {
            wp_send_json_error(__('Author name is required', 'sky360'));
        }
        
        if ($rating < 1 || $rating > 5) {
            wp_send_json_error(__('Invalid rating', 'sky360'));
        }
        ...
        if ($review_id > 0) {
            ...
            if ($result !== false) {
                wp_send_json_success([
                    'message' => __('Review updated successfully', 'sky360'),
                    'review_id' => $review_id
                ]);
            } else {
                wp_send_json_error(__('Failed to update review', 'sky360'));
            }
        } else {
            ...
            if ($result !== false) {
                wp_send_json_success([
                    'message' => __('Review added successfully', 'sky360'),
                    'review_id' => $wpdb->insert_id
                ]);
            } else {
                wp_send_json_error(__('Failed to add review', 'sky360'));
            }
        }
    }
    ...
    public function ajax_upload_review_photo() {
        ...
        if (empty($_FILES['author_photo'])) {
            wp_send_json_error(__('No file uploaded', 'sky360'));
        }
        ...
        if ($movefile && !isset($movefile['error'])) {
            ...
            if (!is_wp_error($attach_id)) {
                ...
                wp_send_json_error(__('Failed to create attachment', 'sky360'));
            }
        } else {
            wp_send_json_error($movefile['error'] ?? __('Upload failed', 'sky360'));
        }
    }
    ...
    public function fetch_new_reviews($place_id = '', $force = false) {
        ...
        if (empty($place_id) || empty($settings['serpapi_key'])) {
            return [
                'success' => false,
                'message' => __('Missing place ID or API key', 'sky360')
            ];
        }
            ...
            if (!empty($stats['last_fetch_time'])) {
                ...
                if ($hours_since < 48) {
                    return [
                        'success' => false,
                        'message' => sprintf(
                            __('Reviews were fetched %.1f hours ago. Next fetch in %.1f hours.', 'sky360'),
                            $hours_since,
                            48 - $hours_since
                        )
                    ];
        ...
        if (empty($reviews)) {
            return [
                'success' => true,
                'message' => __('No new reviews found. All reviews are up to date.', 'sky360'),
                'data' => [
                    'fetched' => [
                        'inserted' => 0,
                        'updated' => 0,
        ...
        return [
            'success' => true,
            'message' => sprintf(
                __('Fetched %d reviews: %d new, %d updated. Total reviews: %d', 'sky360'),
                $result['total'],
                $result['inserted'],
                $result['updated'],
    ...
    private function format_review_data($row) {
        ...
        $relative_time = human_time_diff($review_timestamp, current_time('timestamp')) . ' ' . __('ago', 'sky360');
    ...
    public function ajax_delete_review() {
        ...
        if ($review_id) {
            ...
            if ($result !== false) {
                wp_send_json_success(__('Review hidden successfully', 'sky360'));
            }
        }
        
        wp_send_json_error(__('Failed to hide review', 'sky360'));
    }
    ...
    public function ajax_toggle_review_visibility() {
        ...
        if ($review_id) {
            ...
            if ($result !== false) {
                wp_send_json_success([
                    'message' => $new_visibility ? __('Review shown', 'sky360') : __('Review hidden', 'sky360'),
                    'is_visible' => $new_visibility
                ]);
            }
        }
        
        wp_send_json_error(__('Failed to toggle review visibility', 'sky360'));
    }
