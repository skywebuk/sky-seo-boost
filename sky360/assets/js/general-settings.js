...
jQuery(document).ready(function($) {
    ...
    const duplicateOptions = $('.sky360-settings-card').find('input[name^="sky_seo_settings[duplicate_"]').not(duplicateFeatureEnabled);
    const duplicateSelects = $('.sky360-settings-card').find('select[name^="sky_seo_settings[duplicate_"]');
    ...
    $('.sky360-settings-section').hide();
    $('.sky360-settings-section:first').show();
    
    // Add loading state when saving settings
    $('.sky360-settings-form').on('submit', function() {
        const submitButton = $(this).find('input[type="submit"]');
        submitButton.val('Saving...').prop('disabled', true);
    });
    
    // Tooltip for help text
    $('.sky360-settings-card .description').each(function() {
    ...
    // Auto-save indicator
    let saveTimeout;
    $('.sky360-settings-form input, .sky360-settings-form select').on('change', function() {
        clearTimeout(saveTimeout);
        const $indicator = $('#sky-seo-autosave-indicator');
        
        if (!$indicator.length) {
            $('.sky360-settings-form .submit').append('<span id="sky-seo-autosave-indicator" style="margin-left: 10px; color: #666;">Changes detected</span>');
        } else {
