...
class Sky_SEO_License_Manager {
    ...
    public function plugin_info($result, $action, $args) {
        ...
        if (!isset($args->slug) || $args->slug !== 'sky360') {
            return $result;
        }
    ...
    private function get_plugin_file() {
        ...
        $possible_files = [
            WP_PLUGIN_DIR . '/sky360/sky360.php',
            WP_PLUGIN_DIR . '/sky-seo-boost/sky-seo-boost.php',  // Legacy support
            WP_PLUGIN_DIR . '/sky-seo-boost_beta_v2/sky-seo-boost.php'  // Legacy support
        ];
        ...
        foreach ($plugins as $plugin_file => $plugin_data) {
            if (strpos($plugin_file, 'sky360.php') !== false || strpos($plugin_file, 'sky-seo-boost.php') !== false) {
    ...
    public function disable_plugin_features() {
        ...
        add_action('template_redirect', function() {
            if (is_singular(['sky_areas', 'sky_trending', 'sky_sectors']) || 
                ...
                wp_die(
                    __('This content is not available. The Sky SEO Boost plugin requires a valid license.', 'sky360'),
                    __('License Required', 'sky360'),
                    ['response' => 403]
                );
            }
        });
        ...
        if ($screen && ($screen->id === 'dashboard' || strpos($screen->id, 'sky-seo') !== false || strpos($screen->id, 'plugins') !== false)) {
            ...
            if (get_transient('sky_seo_server_error')) {
                ...
                <div class="notice notice-warning">
                    <p>
                        <strong><?php _e('Sky SEO Boost - License Server Issue:', 'sky360'); ?></strong>
                        <?php _e('Unable to verify license. The plugin is running in grace period mode. If this persists for more than 3 days, the plugin will be disabled.', 'sky360'); ?>
                    </p>
                </div>
                <?php
            } else {
    ...
    public function show_openssl_warning() {
        ...
        if ($screen && strpos($screen->id, 'sky-seo') !== false) {
            ...
            <div class="notice notice-warning">
                <p>
                    <strong><?php _e('Sky SEO Boost - Security Warning:', 'sky360'); ?></strong>
                    <?php _e('The OpenSSL PHP extension is not available. License response signatures cannot be verified, which may pose a security risk.', 'sky360'); ?>
                    <?php _e('Please contact your hosting provider to enable the OpenSSL PHP extension for enhanced security.', 'sky360'); ?>
                </p>
            </div>
            <?php
        }
    ...
    public function render_license_content($active_tab) {
        ...
        <div class="sky-seo-license-settings">
            <div class="sky-seo-license-card">
                <h3><?php _e('Plugin License', 'sky360'); ?></h3>
                ...
                <div class="sky-seo-license-status-box">
                    <div class="status-indicator <?php echo esc_attr($license_status['status']); ?>">
                        <?php if ($license_status['status'] === 'active') : ?>
                            ...
                            <?php _e('License Active', 'sky360'); ?>
                        <?php elseif ($license_status['status'] === 'inactive') : ?>
                            <span class="dashicons dashicons-warning"></span>
                            <?php _e('License Inactive', 'sky360'); ?>
                        <?php else : ?>
                            <span class="dashicons dashicons-no"></span>
                            <?php _e('No License', 'sky360'); ?>
                        <?php endif; ?>
                    </div>
                ...
                <form id="sky-seo-license-form" class="sky-seo-license-form">
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="license_key"><?php _e('License Key', 'sky360'); ?></label>
                            </th>
                            <td>
                                <input type="text" 
                                       id="license_key" 
                                       ...
                                       class="regular-text" 
                                       placeholder="<?php esc_attr_e('Enter your license key', 'sky360'); ?>"
                                ...
                                <?php if ($license_status['status'] === 'active') : ?>
                                    <button type="button" 
                                        ...
                                        <?php _e('Deactivate License', 'sky360'); ?>
                                    </button>
                                    <button type="button" 
                                            id="sky-seo-manual-check" 
                                            class="button button-secondary"
                                            title="<?php esc_attr_e('Manual check is limited to once per hour', 'sky360'); ?>">
                                        <?php _e('Verify Now', 'sky360'); ?>
                                    </button>
                                <?php else : ?>
                                    <button type="button" 
                        ...
                        <?php if ($license_status['status'] === 'active') : ?>
                        <tr>
                            <th scope="row"><?php _e('Licensed Domain', 'sky360'); ?></th>
                            <td>
                        ...
                        <tr>
                            <th scope="row"><?php _e('Last Verified', 'sky360'); ?></th>
                            <td>
                                <?php 
                                $last_check = get_option(self::LAST_CHECK_OPTION, 0);
                                if ($last_check > 0) {
                                    echo esc_html(human_time_diff($last_check, current_time('timestamp'))) . ' ' . __('ago', 'sky360');
                                    
                                    // Show next check time
                                    $next_check = $last_check + self::CHECK_FREQUENCY;
                                    if ($next_check > time()) {
                                        echo '<br><small>' . esc_html__('Next automatic check in', 'sky360') . ' ' .
                        ...
                        <tr>
                            <th scope="row"><?php _e('Updates', 'sky360'); ?></th>
                            <td>
                                <?php
                                $plugin_file = $this->get_plugin_file();
                                if ($plugin_file) {
                                    $plugin_data = get_plugin_data($plugin_file);
                                    echo sprintf(esc_html__('Current version: %s', 'sky360'), esc_html($plugin_data['Version']));
                                    echo ' | <a href="' . esc_url(admin_url('plugins.php')) . '">' . esc_html__('Check for updates', 'sky360') . '</a>';
                                }
                                ?>
                            </td>
                        </tr>
                ...
                <div class="sky-seo-license-info">
                    <h4><?php _e('License Information', 'sky360'); ?></h4>
                    <ul>
                        <li><?php _e('License verification occurs automatically once every 24 hours.', 'sky360'); ?></li>
                        <li><?php _e('If the license server is temporarily unavailable, the plugin will continue working for up to 3 days.', 'sky360'); ?></li>
                        <li><?php _e('Each license is valid for one domain only.', 'sky360'); ?></li>
                        <li><?php _e('Automatic updates are available only for licensed users.', 'sky360'); ?></li>
                        <li><?php printf(__('Need a license? <a href="%s" target="_blank">Purchase one here</a>.', 'sky360'), 'https://skywebdesign.co.uk/sky360'); ?></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        jQuery(document).ready(function($) {
            ...
            $activateBtn.on('click', function() {
                ...
                if (!licenseKey) {
                    showNotice('<?php _e('Please enter a license key.', 'sky360'); ?>', 'error');
                    return;
                }
                
                $activateBtn.prop('disabled', true).text('<?php _e('Activating...', 'sky360'); ?>');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $activateBtn.prop('disabled', false).text('<?php _e('Activate License', 'sky360'); ?>');
                        }
                    },
                    error: function() {
                        showNotice('<?php _e('An error occurred. Please try again.', 'sky360'); ?>', 'error');
                        $activateBtn.prop('disabled', false).text('<?php _e('Activate License', 'sky360'); ?>');
                    }
                });
            });
            ...
            $deactivateBtn.on('click', function() {
                if (!confirm('<?php _e('Are you sure you want to deactivate this license? The plugin will stop working immediately.', 'sky360'); ?>')) {
                    return;
                }
                
                $deactivateBtn.prop('disabled', true).text('<?php _e('Deactivating...', 'sky360'); ?>');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showNotice(response.data.message, 'error');
                            $deactivateBtn.prop('disabled', false).text('<?php _e('Deactivate License', 'sky360'); ?>');
                        }
                    },
                    error: function() {
                        showNotice('<?php _e('An error occurred. Please try again.', 'sky360'); ?>', 'error');
            ...
            $manualCheckBtn.on('click', function() {
                $manualCheckBtn.prop('disabled', true).text('<?php _e('Checking...', 'sky360'); ?>');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $manualCheckBtn.prop('disabled', false).text('<?php _e('Verify Now', 'sky360'); ?>');
                    },
                    error: function() {
                        showNotice('<?php _e('An error occurred. Please try again.', 'sky360'); ?>', 'error');
                        $manualCheckBtn.prop('disabled', false).text('<?php _e('Verify Now', 'sky360'); ?>');
    ...
    public function ajax_activate_license() {
        ...
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Unauthorized access.', 'sky360')]);
        }

        $license_key = isset($_POST['license_key']) ? sanitize_text_field($_POST['license_key']) : '';

        if (empty($license_key)) {
            wp_send_json_error(['message' => __('Please provide a license key.', 'sky360')]);
        }

        // Validate license key format (basic validation)
        if (strlen($license_key) < 10 || strlen($license_key) > 100) {
            wp_send_json_error(['message' => __('Invalid license key format.', 'sky360')]);
        }
    ...
    public function ajax_manual_check() {
        ...
        if ($last_manual_check !== false) {
            wp_send_json_error([
                'message' => __('Manual verification is limited to once per hour. Please wait before trying again.', 'sky360')
            ]);
        }
        ...
        if ($this->is_license_valid()) {
            wp_send_json_success([
                'message' => __('License verified successfully. Status: Active', 'sky360'),
                'reload' => false
            ]);
        } else {
            wp_send_json_error([
                'message' => __('License verification failed. Please check your license key.', 'sky360'),
                'reload' => true
            ]);
        }
    }
    ...
    private function activate_license($license_key) {
        ...
        if (is_wp_error($response)) {
            ...
            $user_message = __('Unable to connect to license server. ', 'sky360');

            if (strpos($error_message, 'cURL') !== false || strpos($error_message, 'curl') !== false) {
                $user_message .= __('Your server may have cURL disabled or restricted.', 'sky360');
            } elseif (strpos($error_message, 'SSL') !== false || strpos($error_message, 'certificate') !== false) {
                $user_message .= __('SSL certificate verification failed. Contact your hosting provider.', 'sky360');
            } elseif (strpos($error_message, 'timed out') !== false || strpos($error_message, 'timeout') !== false) {
                $user_message .= __('Connection timed out. The server may be temporarily unavailable.', 'sky360');
            } elseif (strpos($error_message, 'resolve') !== false || strpos($error_message, 'DNS') !== false) {
                $user_message .= __('DNS resolution failed. Check your server\'s DNS settings.', 'sky360');
            } else {
                $user_message .= __('Please try again later or contact support.', 'sky360');
            }
        ...
        if ($data === null) {
            return [
                'success' => false,
                'message' => __('Invalid response from license server. Please try again later.', 'sky360')
            ];
        }
        ...
        if (isset($data['signature'])) {
            ...
            if (!Sky_SEO_License_Signature_Validator::is_available()) {
                ...
                if (!Sky_SEO_License_Signature_Validator::verify_response($data)) {
                    ...
                    return [
                        'success' => false,
                        'message' => __('License server response verification failed. Please contact support.', 'sky360')
                    ];
                }
        ...
        if (!empty($data['authenticated']) && $data['authenticated'] === true) {
            ...
            return [
                'success' => true,
                'message' => __('License activated successfully! The page will reload to enable all features.', 'sky360')
            ];
        } else {
            $error_message = isset($data['error']) ? $data['error'] : __('Invalid license key or domain mismatch.', 'sky360');
            return [
                'success' => false,
    ...
    private function deactivate_license() {
        ...
        return [
            'success' => true,
            'message' => __('License deactivated successfully. The plugin will be disabled.', 'sky360')
        ];
    }
    ...
    private function get_license_status() {
        ...
        if (empty($license_data['key'])) {
            return [
                'status' => 'none',
                'message' => __('No license key entered. All plugin features are disabled.', 'sky360')
            ];
        }
        ...
        if ($this->is_license_valid()) {
            return [
                'status' => 'active',
                'message' => __('Your license is active and valid.', 'sky360')
            ];
        } else {
            return [
                'status' => 'inactive',
                'message' => __('Your license is invalid or expired. All plugin features are disabled.', 'sky360')
            ];
        }
    }
