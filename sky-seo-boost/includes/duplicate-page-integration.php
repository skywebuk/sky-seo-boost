...
class Sky_SEO_Duplicate_Post_Feature {
    ...
    public function add_duplicate_link($actions, $post) {
        ...
        $title = __('Duplicate this item', 'sky360');
        
        $actions['duplicate'] = '<a href="' . esc_url($duplicate_url) . '" title="' . esc_attr($title) . '">' . __('Duplicate', 'sky360') . '</a>';
    ...
    public function duplicate_post() {
        ...
        if (!isset($_GET['post']) || !isset($_GET['action']) || $_GET['action'] !== 'sky_seo_duplicate_post') {
            wp_die(__('No post to duplicate has been supplied!', 'sky360'));
        }
        ...
        if (!current_user_can('edit_post', $post_id)) {
            wp_die(__('You do not have permission to duplicate this post.', 'sky360'));
        }
    ...
    private function create_duplicate($post_id) {
        ...
        if (!$post) {
            return new WP_Error('post_not_found', __('Post not found', 'sky360'));
        }
        ...
        if (function_exists('icl_t')) {
            $title_prefix = icl_t('sky360', 'Duplicate: Default Prefix', $title_prefix);
            $title_suffix = icl_t('sky360', 'Duplicate: Default Suffix', $title_suffix);
        }
    ...
    public function register_bulk_duplicate($bulk_actions) {
        $bulk_actions['duplicate'] = __('Duplicate', 'sky360');
        return $bulk_actions;
    }
    ...
    public function add_admin_bar_link($wp_admin_bar) {
        ...
        $args = [
            ...
            'title' => __('Duplicate This', 'sky360'),
    ...
    public function add_bulk_duplicate_script() {
        ...
        jQuery(document).ready(function($) {
            ...
            $(document).on('click', '.sky-seo-quick-duplicate', function(e) {
                ...
                $link.text('<?php _e('Duplicating...', 'sky360'); ?>');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            $link.text('<?php _e('Duplicated!', 'sky360'); ?>');
                            setTimeout(function() {
                                location.reload();
                            }, 1000);
                        } else {
                            $link.text('<?php _e('Error', 'sky360'); ?>');
                            alert(response.data || '<?php _e('An error occurred', 'sky360'); ?>');
                        }
                    },
                    error: function() {
                        $link.text('<?php _e('Error', 'sky360'); ?>');
                        alert('<?php _e('An error occurred', 'sky360'); ?>');
                    }
                });
            });
        });
    ...
    public function ajax_duplicate_post() {
        ...
        if (!check_ajax_referer('sky_seo_duplicate_ajax', 'nonce', false)) {
            wp_send_json_error(__('Invalid security token', 'sky360'));
        }
        
        $post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : 0;
        
        if (!$post_id) {
            wp_send_json_error(__('Invalid post ID', 'sky360'));
        }
        ...
        wp_send_json_success([
            'message' => __('Post duplicated successfully', 'sky360'),
