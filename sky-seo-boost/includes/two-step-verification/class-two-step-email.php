...
class Sky_SEO_Two_Step_Email {
    ...
    public function get_label() {
        return __('Email Verification', 'sky360');
    }
    ...
    public function generate_and_email_token($user) {
        ...
        $subject = wp_strip_all_tags(sprintf(__('[%s] Two-Step Verification Code', 'sky360'), $site_name));
    ...
    public function authentication_page($user) {
        ...
        if ($is_resend && isset($_POST['sky-seo-two-step-resend-nonce']) &&
            ...
            if ($this->generate_and_email_token($user)) {
                echo '<p class="message">' . esc_html__('A new verification code has been sent to your email address.', 'sky360') . '</p>';
            } else {
                echo '<p class="message error">' . esc_html__('Failed to send verification code. Please try again.', 'sky360') . '</p>';
            }
        ...
        <div class="sky-seo-two-step-form">
            <h2><?php esc_html_e('Email Verification Required', 'sky360'); ?></h2>

            <p>
                <?php
                printf(
                    /* translators: %s: User email address */
                    esc_html__('A verification code has been sent to %s. Please enter the code below to complete your login.', 'sky360'),
            ...
            <?php if ($failed_attempts > 0) : ?>
                <p class="message error">
                    ...
                    printf(
                        ...
                        esc_html(_n('Invalid code. %d failed attempt.', 'Invalid code. %d failed attempts.', $failed_attempts, 'sky360')),
                        $failed_attempts
                    );
                    ?>
                </p>
            ...
            <label for="sky-seo-two-step-code">
                <?php esc_html_e('Verification Code:', 'sky360'); ?>
            </label>
            <input type="text"
            ...
            <p style="margin-top: 16px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="checkbox"
                           ...
                           value="1"
                           style="margin: 0;" />
                    <span><?php esc_html_e('Remember this device for 14 days', 'sky360'); ?></span>
                </label>
            </p>

            <div class="sky-seo-two-step-resend">
                <p><?php esc_html_e("Didn't receive the code?", 'sky360'); ?></p>
                <!-- Use type="button" to prevent Enter key from triggering resend -->
                <button type="button"
                        id="sky-seo-two-step-resend-btn"
                        class="button button-secondary">
                    <?php esc_html_e('Resend Verification Code', 'sky360'); ?>
                </button>
                <input type="hidden"
    ...
    public function user_settings($user) {
        ...
        <div class="sky-seo-two-step-email-settings">
            <p>
                <strong><?php esc_html_e('Email Verification Status:', 'sky360'); ?></strong>
                <?php if ($is_enabled) : ?>
                    <span style="color: #00a32a;"><?php esc_html_e('Enabled', 'sky360'); ?></span>
                <?php else : ?>
                    <span style="color: #d63638;"><?php esc_html_e('Disabled', 'sky360'); ?></span>
                <?php endif; ?>
            </p>
            ...
            <p class="description">
                ...
                printf(
                    /* translators: %s: User email address */
                    esc_html__('Verification codes will be sent to: %s', 'sky360'),
                    '<strong>' . esc_html($user->user_email) . '</strong>'
                );
                ?>
            </p>
