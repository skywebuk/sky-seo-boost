...
class Sky_SEO_Two_Step_Core {
    ...
    private static function process_two_step_form($user) {
        try {
            ...
            if (!self::$email_provider) {
                ...
                return new WP_Error(
                    'two_step_error',
                    __('Two-step verification system error. Please contact the administrator.', 'sky-seo-boost')
                );
            }
            ...
            if (!isset($_POST['sky-seo-two-step-nonce']) ||
                ...
                return new WP_Error(
                    'invalid_nonce',
                    __('Session expired. Please log in again.', 'sky-seo-boost')
                );
            }
            ...
            if (self::is_rate_limited($user)) {
                ...
                return new WP_Error(
                    ...
                    sprintf(
                        ...
                        __('Too many failed attempts. Please wait %d seconds before trying again.', 'sky-seo-boost'),
                        $delay
                    )
                );
            }
            ...
            return new WP_Error(
                'invalid_code',
                __('Invalid verification code. Please try again.', 'sky-seo-boost')
            );
        } catch (Exception $e) {
            error_log('Sky SEO Boost: Error in process_two_step_form(): ' . $e->getMessage());
            return new WP_Error(
                'two_step_error',
                __('An error occurred during verification. Please try again.', 'sky-seo-boost')
            );
        }
    }
    ...
    private static function show_two_step_form($user, $username = '', $password = '') {
        try {
            ...
            if (!self::$email_provider) {
                ...
                wp_die(
                    __('Two-step verification system error. Please contact the administrator.', 'sky-seo-boost'),
                    __('Authentication Error', 'sky-seo-boost'),
                    array('response' => 500)
                );
            }
            ...
            login_header(
                __('Two-Step Verification', 'sky-seo-boost'),
                '',
                ''
            );
            ...
            <form name="sky-seo-two-step-form" id="sky-seo-two-step-form" action="<?php echo esc_url(site_url('wp-login.php', 'login_post')); ?>" method="post">
                ...
                <p class="submit">
                    <input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Verify', 'sky-seo-boost'); ?>" />
                </p>
            ...
            <p id="backtoblog">
                <a href="<?php echo esc_url(home_url('/')); ?>"><?php
                    printf(
                        ...
                        esc_html__('&larr; Go to %s', 'sky-seo-boost'),
            ...
            wp_die(
                __('An error occurred during authentication. Please try again or contact the administrator.', 'sky-seo-boost'),
                __('Authentication Error', 'sky-seo-boost'),
                array('response' => 500)
            );
        }
    ...
    public static function user_profile_fields($user) {
        ...
        ?>
        <h2><?php esc_html_e('Two-Step Verification', 'sky-seo-boost'); ?></h2>
        <table class="form-table">
            <tr>
                <th><?php esc_html_e('Email Verification', 'sky-seo-boost'); ?></th>
                <td>
                    <?php if ($is_enforced) : ?>
                        <p>
                            <strong><?php esc_html_e('Status:', 'sky-seo-boost'); ?></strong>
                            <span style="color: #00a32a;"><?php esc_html_e('Enforced by administrator', 'sky-seo-boost'); ?></span>
                        </p>
                        <p class="description">
                            <?php esc_html_e('Two-step verification is required for your user role and cannot be disabled.', 'sky-seo-boost'); ?>
                        </p>
                    <?php else : ?>
                        <label for="sky_seo_two_step_enabled">
                            ...
                            <?php esc_html_e('Enable two-step verification via email', 'sky-seo-boost'); ?>
                        </label>
                        <p class="description">
                            <?php esc_html_e('When enabled, you will receive a verification code via email each time you log in.', 'sky-seo-boost'); ?>
                        </p>
                    <?php endif; ?>
            ...
            <tr>
                <th><?php esc_html_e('Trusted Devices', 'sky-seo-boost'); ?></th>
                <td>
                    <?php
                    ...
                    ?>
                        <p><?php esc_html_e('These devices are trusted and will not require 2FA verification:', 'sky-seo-boost'); ?></p>
                        <table class="widefat striped" style="max-width: 600px; margin-top: 10px;">
                            <thead>
                                <tr>
                                    <th><?php esc_html_e('Device', 'sky-seo-boost'); ?></th>
                                    <th><?php esc_html_e('Added', 'sky-seo-boost'); ?></th>
                                    <th><?php esc_html_e('Expires', 'sky-seo-boost'); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        <p style="margin-top: 10px;">
                            <button type="submit" name="sky_seo_two_step_clear_devices" value="1" class="button" onclick="return confirm('<?php esc_attr_e('Are you sure you want to remove all trusted devices? You will need to verify with 2FA on all devices.', 'sky-seo-boost'); ?>');">
                                <?php esc_html_e('Remove All Trusted Devices', 'sky-seo-boost'); ?>
                            </button>
                        </p>
                    <?php else : ?>
                        <p class="description">
                            <?php esc_html_e('No trusted devices. Use "Remember this device" when logging in to skip 2FA verification for 14 days.', 'sky-seo-boost'); ?>
                        </p>
                    <?php endif; ?>
                </td>
            </tr>
    ...
    private static function parse_user_agent($user_agent) {
        if (empty($user_agent)) {
            return __('Unknown Device', 'sky-seo-boost');
        }
        ...
        return __('Unknown Browser', 'sky-seo-boost');
    }
    ...
    public static function handle_2fa_redirect() {
        ...
        if (!isset($_GET['user_id']) || !isset($_GET['interim_token'])) {
            wp_die(
                __('Invalid authentication request. Please try logging in again.', 'sky-seo-boost'),
                __('Authentication Error', 'sky-seo-boost'),
                array('response' => 400)
            );
        }
        ...
        if (!$user) {
            wp_die(
                __('Invalid user. Please try logging in again.', 'sky-seo-boost'),
                __('Authentication Error', 'sky-seo-boost'),
                array('response' => 400)
            );
        }
        ...
        if (!$token_data) {
            login_header(
                __('Two-Step Verification', 'sky-seo-boost'),
                '<p class="message error">' . __('Authentication session expired. Please try logging in again.', 'sky-seo-boost') . '</p>'
            );
            login_footer();
            exit;
        }
