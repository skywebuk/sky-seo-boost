...
function sky_seo_add_internal_linking_metabox() {
   ...
   add_meta_box(
       ...
       __('Internal Linking Suggestions', 'sky-seo-boost'),
...
function sky_seo_render_internal_linking_metabox($post) {
   ...
   if (empty($suggestions)) {
       echo '<p>' . esc_html__('No related posts found. Try linking to other relevant content.', 'sky-seo-boost') . '</p>';
   } else {
       echo '<p>' . esc_html__('Suggested posts to link to:', 'sky-seo-boost') . '</p>';
       echo '<ul>';
       ...
       echo '</ul>';
       echo '<p class="description">' . esc_html__('Click a link to copy its URL.', 'sky-seo-boost') . '</p>';
   }
}
...
function sky_seo_add_dashboard_widget() {
   wp_add_dashboard_widget(
       ...
       __('Sky SEO Boost: Top Pages', 'sky-seo-boost'),
...
function sky_seo_add_clicks_column($columns) {
    ...
    foreach ($columns as $key => $value) {
        ...
        if ($key === 'title') {
            $new_columns['sky_seo_clicks'] = '<span class="dashicons dashicons-chart-bar" title="' . __('Total Clicks', 'sky-seo-boost') . '"></span> ' . __('Clicks', 'sky-seo-boost');
        }
    }
    return $new_columns;
}
    ...
    if ($column === 'sky_seo_clicks') {
        ...
        if ($total_clicks > 0 && ($today_clicks || $yesterday_clicks)) {
            ...
            if ($today_clicks > $yesterday_clicks) {
                echo '<span style="color: #15803d;" title="' . __('Trending up', 'sky-seo-boost') . '">↑</span>';
            } elseif ($today_clicks < $yesterday_clicks && $yesterday_clicks > 0) {
                echo '<span style="color: #dc2626;" title="' . __('Trending down', 'sky-seo-boost') . '">↓</span>';
            }
            echo '</div>';
        }
        echo '</div>';
...
function sky_seo_render_dashboard_widget() {
   ...
   if ($top_pages) {
       ...
       foreach ($top_pages as $page) {
           ...
           echo ' (' . number_format_i18n($page->total_clicks) . ' ' . __('clicks', 'sky-seo-boost') . ')';
           echo '</li>';
       }
       echo '</ul>';
   } else {
       echo '<p>' . __('No data available yet.', 'sky-seo-boost') . '</p>';
   }

   echo '<p class="description">' . __('Last 30 days', 'sky-seo-boost') . '</p>';
}
