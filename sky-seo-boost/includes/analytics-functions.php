...
function sky_seo_analytics_init() {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'export_analytics') {
        ...
        if (check_admin_referer('sky_seo_export_analytics')) {
            ...
            wp_die(__('Security check failed. Please try again.', 'sky-seo-boost'));
        }
    }
...
<div class="sky-seo-dashboard-header">
    <div class="sky-header-top">
        <h2><?php _e('Analytics Dashboard', 'sky-seo-boost'); ?></h2>
        
<div class="sky-powered-by">
    <a href="https://skywebdesign.co.uk/" target="_blank">
        <span><?php _e('Powered by Sky Web', 'sky-seo-boost'); ?></span>
    ...
    <input type="hidden" name="page" value="sky-seo-boost">

                    <!-- Quick Date Presets -->
                    <div class="sky-seo-date-presets">
                        <select name="date_range" id="date_range" class="sky-seo-date-preset">
                            <option value="today" <?php selected($date_range, 'today'); ?>><?php _e('Today', 'sky-seo-boost'); ?></option>
                            <option value="yesterday" <?php selected($date_range, 'yesterday'); ?>><?php _e('Yesterday', 'sky-seo-boost'); ?></option>
                            <option value="last_7_days" <?php selected($date_range, 'last_7_days'); ?>><?php _e('Last 7 Days', 'sky-seo-boost'); ?></option>
                            <option value="last_30_days" <?php selected($date_range, 'last_30_days'); ?>><?php _e('Last 30 Days', 'sky-seo-boost'); ?></option>
                            <option value="last_90_days" <?php selected($date_range, 'last_90_days'); ?>><?php _e('Last 90 Days', 'sky-seo-boost'); ?></option>
                            <option value="this_month" <?php selected($date_range, 'this_month'); ?>><?php _e('This Month', 'sky-seo-boost'); ?></option>
                            <option value="last_month" <?php selected($date_range, 'last_month'); ?>><?php _e('Last Month', 'sky-seo-boost'); ?></option>
                            <option value="custom" <?php selected($date_range, 'custom'); ?>><?php _e('Custom Range', 'sky-seo-boost'); ?></option>
                        </select>
                    </div>
                    ...
                    <div class="sky-seo-custom-dates" <?php echo $date_range !== 'custom' ? 'style="display:none;"' : ''; ?>>
                        ...
                        <span><?php _e('to', 'sky-seo-boost'); ?></span>
                    ...
                    <select name="post_type_filter" class="sky-seo-filter-select">
                        <option value="all" <?php selected($selected_post_type, 'all'); ?>><?php _e('All Content Types', 'sky-seo-boost'); ?></option>
                        <optgroup label="<?php _e('Sky SEO Post Types', 'sky-seo-boost'); ?>">
                            <?php // Iterate through available sky_seo post types if they are registered ?>
                            <option value="sky_areas" <?php selected($selected_post_type, 'sky_areas'); ?>><?php _e('Areas', 'sky-seo-boost'); ?></option>
                            <option value="sky_trending" <?php selected($selected_post_type, 'sky_trending'); ?>><?php _e('Trending Searches', 'sky-seo-boost'); ?></option>
                            <option value="sky_sectors" <?php selected($selected_post_type, 'sky_sectors'); ?>><?php _e('Sectors', 'sky-seo-boost'); ?></option>
                        </optgroup>
                        <optgroup label="<?php _e('WordPress Content', 'sky-seo-boost'); ?>">
                            <option value="page" <?php selected($selected_post_type, 'page'); ?>><?php _e('Pages', 'sky-seo-boost'); ?></option>
                            <option value="post" <?php selected($selected_post_type, 'post'); ?>><?php _e('Posts', 'sky-seo-boost'); ?></option>
                        </optgroup>
                        <?php if (class_exists('WooCommerce')) : ?>
                            <optgroup label="<?php _e('WooCommerce', 'sky-seo-boost'); ?>">
                                <option value="product" <?php selected($selected_post_type, 'product'); ?>><?php _e('Products', 'sky-seo-boost'); ?></option>
                            </optgroup>
                        <?php endif; ?>
                    </select>
                    ...
                    <label class="sky-seo-compare-toggle">
                        ...
                        <?php _e('Compare Periods', 'sky-seo-boost'); ?>
                    </label>

                    <button type="submit" class="button button-primary"><?php _e('Apply', 'sky-seo-boost'); ?></button>
                    <?php
                    // Correctly generate the export URL with nonce and current filters
$export_url_args = [
    'page'               => 'sky-seo-boost', // Add this line
    ...
    '_wpnonce'           => wp_create_nonce('sky_seo_export_analytics') // Add nonce for security
                    ...
                    $export_url = add_query_arg($export_url_args, admin_url('admin.php?page=sky-seo-settings&tab=analytics-tab'));
                    $export_url = wp_nonce_url($export_url, 'sky_seo_export_analytics');
                    ?>
                    <a href="<?php echo esc_url($export_url); ?>" class="button"><?php _e('Export CSV', 'sky-seo-boost'); ?></a>
                </form>
            </div>
        </div>
...
<div class="sky-seo-traffic-quality">
    <div class="sky-seo-quality-header">
        <h3>
            ...
            <?php _e('Traffic Quality Overview', 'sky-seo-boost'); ?>
        </h3>
        <span class="description"><?php echo sprintf(__('Period: %s to %s', 'sky-seo-boost'), $start_date, $end_date); ?></span>
    </div>
    ...
    <div class="sky-seo-quality-metrics">
        <div class="sky-seo-quality-metric human-traffic">
            ...
            <h4><?php _e('Real Visitor Traffic', 'sky-seo-boost'); ?></h4>
            ...
            <div class="description"><?php _e('Verified human visitors', 'sky-seo-boost'); ?></div>
        </div>
        ...
        <div class="sky-seo-quality-metric bot-traffic">
            ...
            <h4><?php _e('BOT TRAFFIC', 'sky-seo-boost'); ?></h4>
            ...
            <div class="description"><?php _e('Search engines & crawlers', 'sky-seo-boost'); ?></div>
        </div>
        ...
        <div class="sky-seo-quality-metric suspicious-traffic">
            ...
            <h4><?php _e('SUSPICIOUS TRAFFIC', 'sky-seo-boost'); ?></h4>
            ...
            <div class="description"><?php _e('Potential spam/automated', 'sky-seo-boost'); ?></div>
        </div>
        ...
        <div class="sky-seo-quality-metric total-traffic">
            ...
            <h4><?php _e('TOTAL TRAFFIC', 'sky-seo-boost'); ?></h4>
            <div class="value"><?php echo number_format_i18n($traffic_quality['total_clicks']); ?></div>
            <div class="description"><?php _e('All recorded clicks', 'sky-seo-boost'); ?></div>
        </div>
    </div>
    ...
    <div class="sky-seo-traffic-breakdown">
        <div class="sky-seo-breakdown-bars">
            <?php if ($traffic_quality['human_percentage'] > 0) : ?>
                <div class="sky-seo-breakdown-segment human" 
                     ...
                     title="<?php echo esc_attr(sprintf(__('Human: %s%%', 'sky-seo-boost'), number_format_i18n($traffic_quality['human_percentage'], 1))); ?>">
            ...
            <?php if ($traffic_quality['bot_percentage'] > 0) : ?>
                <div class="sky-seo-breakdown-segment bot" 
                     ...
                     title="<?php echo esc_attr(sprintf(__('Bots: %s%%', 'sky-seo-boost'), number_format_i18n($traffic_quality['bot_percentage'], 1))); ?>">
            ...
            <?php if ($traffic_quality['suspicious_percentage'] > 0) : ?>
                <div class="sky-seo-breakdown-segment suspicious" 
                     ...
                     title="<?php echo esc_attr(sprintf(__('Suspicious: %s%%', 'sky-seo-boost'), number_format_i18n($traffic_quality['suspicious_percentage'], 1))); ?>">
        ...
        <div class="sky-seo-breakdown-legend">
            <div class="sky-seo-legend-item">
                ...
                <span><?php _e('Human Traffic', 'sky-seo-boost'); ?></span>
            </div>
            <div class="sky-seo-legend-item">
                <div class="sky-seo-legend-color" style="background: #ef4444;"></div>
                <span><?php _e('Bot Traffic', 'sky-seo-boost'); ?></span>
            </div>
            <div class="sky-seo-legend-item">
                <div class="sky-seo-legend-color" style="background: #f59e0b;"></div>
                <span><?php _e('Suspicious', 'sky-seo-boost'); ?></span>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="sky-seo-metrics-grid">
            ...
            <div class="sky-seo-metric-card">
                ...
                <div class="sky-seo-metric-content">
                    <h3><?php _e('Total Clicks', 'sky-seo-boost'); ?>
                        <span style="font-size: 12px; color: #666; font-weight: normal;">
                            (<?php echo $show_all_traffic ? __('All', 'sky-seo-boost') : __('Human', 'sky-seo-boost'); ?>)
                        </span>
                    </h3>
            ...
            <div class="sky-seo-metric-card">
                ...
                <div class="sky-seo-metric-content">
                    <h3><?php _e('Unique Content Viewed', 'sky-seo-boost'); ?></h3>
            ...
            <div class="sky-seo-metric-card">
                ...
                <div class="sky-seo-metric-content">
                    <h3><?php _e('Avg. Clicks/Content', 'sky-seo-boost'); ?></h3>
            ...
            <div class="sky-seo-metric-card">
                ...
                <div class="sky-seo-metric-content">
                    <h3><?php _e('Growth Rate', 'sky-seo-boost'); ?></h3>
...
<div class="sky-seo-top-sources">
    <div class="sky-seo-sources-header">
        <h3>
            ...
            <?php _e('Top Traffic Sources', 'sky-seo-boost'); ?>
        </h3>
        <span class="sky-seo-sources-subtitle"><?php _e('Where your visitors come from', 'sky-seo-boost'); ?></span>
    </div>
    ...
    <div class="sky-seo-sources-grid">
        ...
        <?php if ($show_all_traffic) : ?>
            <div class="sky-seo-bot-activity">
                <h3><?php _e('Recent Bot Activity', 'sky-seo-boost'); ?></h3>
                <table class="sky-seo-bot-table">
                    <thead>
                        <tr>
                            <th><?php _e('Bot Type', 'sky-seo-boost'); ?></th>
                            <th><?php _e('Name', 'sky-seo-boost'); ?></th>
                            <th><?php _e('Visits', 'sky-seo-boost'); ?></th>
                            <th><?php _e('Last Seen', 'sky-seo-boost'); ?></th>
                            <th><?php _e('Purpose', 'sky-seo-boost'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        if (!empty($bot_activity)) :
                            foreach ($bot_activity as $bot) :
                                ...
                                <tr>
                                    ...
                                    <td><?php echo human_time_diff(strtotime($bot->last_seen), current_time('timestamp')) . ' ' . __('ago', 'sky-seo-boost'); ?></td>
                            ...
                            <tr>
                                <td colspan="5" style="text-align: center; color: #999;">
                                    <?php _e('No bot activity detected in this period.', 'sky-seo-boost'); ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
        ...
        <div class="sky-seo-charts-row">
            ...
            <div class="sky-seo-chart-card">
                <h3><?php _e('Traffic Sources', 'sky-seo-boost'); ?></h3>
            ...
            <!-- Content Performance -->
            <div class="sky-seo-chart-card">
                <h3><?php _e('Content Type Performance', 'sky-seo-boost'); ?></h3>
            ...
            <div class="sky-seo-chart-card sky-seo-chart-wide">
                <h3><?php _e('Click Trends', 'sky-seo-boost'); ?></h3>
        ...
        <div class="sky-seo-chart-card sky-seo-chart-wide">
            <h3><?php _e('Geographic Performance', 'sky-seo-boost'); ?></h3>
        ...
        <div class="sky-seo-content-table-card">
            <div class="sky-seo-table-header">
                <h3><?php _e('Top Performing Content', 'sky-seo-boost'); ?></h3>
                <div class="sky-seo-table-actions">
                    <input type="text" id="sky-seo-content-search" placeholder="<?php _e('Search content...', 'sky-seo-boost'); ?>" class="sky-seo-search-input">
                    <select id="sky-seo-content-type-filter" class="sky-seo-filter-select">
                        <option value=""><?php _e('All Types', 'sky-seo-boost'); ?></option>
                        <option value="sky_areas"><?php _e('Areas', 'sky-seo-boost'); ?></option>
                        <option value="sky_trending"><?php _e('Trending', 'sky-seo-boost'); ?></option>
                        <option value="sky_sectors"><?php _e('Sectors', 'sky-seo-boost'); ?></option>
                        <option value="page"><?php _e('Pages', 'sky-seo-boost'); ?></option>
                        <option value="post"><?php _e('Posts', 'sky-seo-boost'); ?></option>
                        <?php if (class_exists('WooCommerce')) : ?>
                            <option value="product"><?php _e('Products', 'sky-seo-boost'); ?></option>
                        <?php endif; ?>
                    </select>
                    <select id="sky-seo-content-traffic-filter" class="sky-seo-filter-select">
                        <option value=""><?php _e('All Traffic', 'sky-seo-boost'); ?></option>
                        <option value="google"><?php _e('Google Traffic', 'sky-seo-boost'); ?></option>
                        <option value="social"><?php _e('Social Traffic', 'sky-seo-boost'); ?></option>
                        <option value="direct"><?php _e('Direct Traffic', 'sky-seo-boost'); ?></option>
                    </select>
                    <select id="sky-seo-content-sort" class="sky-seo-filter-select">
                        <option value="total_clicks"><?php _e('Sort by Total Clicks', 'sky-seo-boost'); ?></option>
                        <option value="google_clicks"><?php _e('Sort by Google Clicks', 'sky-seo-boost'); ?></option>
                        <option value="social_clicks"><?php _e('Sort by Social Clicks', 'sky-seo-boost'); ?></option>
                        <option value="direct_clicks"><?php _e('Sort by Direct Clicks', 'sky-seo-boost'); ?></option>
                        <option value="recent"><?php _e('Sort by Recent', 'sky-seo-boost'); ?></option>
                    </select>
                </div>
            </div>
            ...
            <table class="sky-seo-analytics-table widefat">
                <thead>
                    <tr>
                        ...
                        <th class="column-type"><?php _e('Type', 'sky-seo-boost'); ?></th>
                        <th class="column-clicks sortable" data-sort="clicks"><?php _e('Total Clicks', 'sky-seo-boost'); ?></th>
                        <th class="column-google sortable" data-sort="google"><?php _e('Google', 'sky-seo-boost'); ?></th>
                        <th class="column-social sortable" data-sort="social"><?php _e('Social', 'sky-seo-boost'); ?></th>
                        <th class="column-direct sortable" data-sort="direct"><?php _e('Direct', 'sky-seo-boost'); ?></th>
                        <th class="column-trend"><?php _e('Trend', 'sky-seo-boost'); ?></th>
                        <th class="column-actions"><?php _e('Actions', 'sky-seo-boost'); ?></th>
                    </tr>
                </thead>
                        ...
                        <tr data-post-id="<?php echo esc_attr($content->ID); ?>">
                            <td class="column-title">
                                ...
                                <div class="row-actions">
                                    <span class="edit"><a href="<?php echo esc_url(get_edit_post_link($content->ID)); ?>"><?php _e('Edit', 'sky-seo-boost'); ?></a> | </span>
                                    <span class="view"><a href="<?php echo esc_url(get_permalink($content->ID)); ?>" target="_blank"><?php _e('View', 'sky-seo-boost'); ?></a></span>
                                </div>
                            </td>
...
function sky_seo_ajax_get_post_details() {
    ...
    if (!$post_title) {
        $post_title = __('Content not found', 'sky-seo-boost'); // Fallback title
    }
    ...
    <div class="sky-seo-post-details">
        ...
        <div class="sky-seo-country-breakdown">
            <h4><?php _e('Traffic by Country', 'sky-seo-boost'); ?></h4>
            <?php if (!empty($countries)) : ?>
                <table class="sky-seo-country-table">
                    <thead>
                        <tr>
                            <th><?php _e('Country', 'sky-seo-boost'); ?></th>
                            <th><?php _e('Views', 'sky-seo-boost'); ?></th>
                            <th><?php _e('Percentage', 'sky-seo-boost'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        foreach ($countries as $country) :
                            ...
                            $country_name = $country->country_name ?: __('Unknown', 'sky-seo-boost');
                            ?>
                            <tr>
                                <td>
            ...
            <?php else : ?>
                <p class="sky-seo-no-data"><?php _e('No country data available for this post.', 'sky-seo-boost'); ?></p>
            <?php endif; ?>
        </div>
    </div>
    <?php
...
function sky_seo_get_analytics_data($start_date, $end_date, $post_type = 'all', $human_only = true) {
    ...
    $data['traffic_sources'] = [
        ['label' => __('Google', 'sky-seo-boost'), 'value' => $data['google_clicks'], 'color' => '#4285F4'],
        ['label' => __('Social', 'sky-seo-boost'), 'value' => $data['social_clicks'], 'color' => '#8B5CF6'],
        ['label' => __('Direct', 'sky-seo-boost'), 'value' => $data['direct_clicks'], 'color' => '#F59E0B'],
    ];
...
function sky_seo_get_post_type_label($post_type) {
    $labels = [
        'sky_areas' => __('Area', 'sky-seo-boost'),
        'sky_trending' => __('Trending', 'sky-seo-boost'),
        'sky_sectors' => __('Sector', 'sky-seo-boost'),
        'page' => __('Page', 'sky-seo-boost'),
        'post' => __('Post', 'sky-seo-boost'),
        'product' => __('Product', 'sky-seo-boost'),
    ];
