...
class Sky_SEO_Business_API_Handler {
    ...
    public function can_make_api_calls($calls = 1) {
        ...
        if ($stats['monthly_used'] + $calls > $stats['monthly_limit']) {
            return [
                'allowed' => false,
                'message' => sprintf(
                    __('Monthly API limit reached (%d/%d). Resets on the 1st.', 'sky360'),
                    $stats['monthly_used'],
                    $stats['monthly_limit']
                )
            ];
        ...
        if ($stats['daily_used'] + $calls > $stats['daily_limit']) {
            ...
            if ($projected_usage > $stats['monthly_limit'] * 0.9) {
                return [
                    'allowed' => false,
                    'message' => sprintf(
                        __('Daily limit reached to preserve monthly quota. Used %d/%d this month.', 'sky360'),
                        $stats['monthly_used'],
                        $stats['monthly_limit']
                    )
                ];
    ...
    public function ajax_test_api() {
        ...
        if (empty($settings['serpapi_key'])) {
            wp_send_json_error(__('Please enter your SerpApi key', 'sky360'));
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            wp_send_json_error(__('Invalid response from API', 'sky360'));
        }
        ...
        $message = sprintf(
            __('✓ Connected! You have %d/%d searches remaining this month from SerpApi.', 'sky360'),
    ...
    public function ajax_fetch_reviews() {
        ...
        if (!class_exists('Sky_SEO_Reviews_Database')) {
            wp_send_json_error(__('Reviews database not available', 'sky360'));
        }
    ...
    public function ajax_get_review_text() {
        ...
        if (!$review_id) {
            wp_send_json_error(__('Invalid review ID', 'sky360'));
        }
        ...
        if ($review_text !== null) {
            ...
            wp_send_json_error(__('Review not found', 'sky360'));
        }
    }
    ...
    public function ajax_edit_review() {
        ...
        if ($result !== false) {
            wp_send_json_success(__('Review updated successfully', 'sky360'));
        } else {
    ...
    public function ajax_update_google_metadata() {
        ...
        if (empty($place_id) || empty($api_key)) {
            wp_send_json_error(__('Missing Place ID or API key', 'sky360'));
        }
        ...
        if (class_exists('Sky_SEO_Business_Info_Elementor_Widget')) {
            ...
            if ($metadata !== false && isset($metadata['total_reviews'])) {
                $message = sprintf(
                    __('✓ Metadata updated! Reviews: %d, Rating: %.1f', 'sky360'),
        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($data['place_info']) && !isset($data['error'])) {
                ...
                wp_send_json_error(__('No place info found in response', 'sky360'));
            }
        } else {
            wp_send_json_error($response->get_error_message());
        }
