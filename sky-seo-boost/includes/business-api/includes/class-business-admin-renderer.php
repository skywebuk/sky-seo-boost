...
class Sky_SEO_Business_Admin_Renderer {
    ...
    private function render_reviews_tab() {
        if (!class_exists('Sky_SEO_Reviews_Database')) {
            wp_die(__('Reviews database not available', 'sky-seo-boost'));
        }
        ...
        <a href="#" class="page-title-action" id="add-manual-review">
            <?php _e('Add Manual Review', 'sky-seo-boost'); ?>
        </a>
        
        <?php if (!empty($place_id)) : ?>
        <a href="#" class="page-title-action" id="refresh-reviews">
            <?php _e('Refresh Google Reviews', 'sky-seo-boost'); ?>
        </a>
        <?php endif; ?>
    ...
    private function render_dashboard($usage_stats, $review_stats) {
        ...
        <div class="api-usage-dashboard">
            <h2><?php _e('Overview', 'sky-seo-boost'); ?></h2>
            <div class="usage-stats-grid">
                <div class="usage-stat">
                    <h4><?php _e('API Usage', 'sky-seo-boost'); ?></h4>
                    <p class="stat-value">
                    ...
                    <p class="stat-subtitle">
                        ...
                        printf(__('%d days left • %.1f/day recommended', 'sky-seo-boost'), $remaining_days, $recommended_per_day);
                        ?>
                    </p>
                </div>
                ...
                <?php if ($review_stats) : ?>
                <div class="usage-stat">
                    <h4><?php _e('Reviews', 'sky-seo-boost'); ?></h4>
                    <p class="stat-value">
                    ...
                    <p class="stat-subtitle">
                        ...
                        <span><?php echo $review_stats['average_rating']; ?> <?php _e('average', 'sky-seo-boost'); ?></span>
                    </p>
                    <?php if (isset($review_stats['platform_breakdown'])) : ?>
                    <p class="stat-subtitle" style="margin-top: 10px; font-size: 11px;">
                        <?php 
                        echo sprintf(
                            __('Google: %d | FB: %d | Trust: %d', 'sky-seo-boost'),
                ...
                <div class="usage-stat">
                    <h4><?php _e('Last Sync', 'sky-seo-boost'); ?></h4>
                    <p class="stat-value" style="font-size: 24px;">
                        <?php 
                        if ($review_stats['last_fetch_time']) {
                            echo human_time_diff(strtotime($review_stats['last_fetch_time'])) . ' ' . __('ago', 'sky-seo-boost');
                        } else {
                    ...
                    <a href="<?php echo admin_url('admin.php?page=sky-seo-business-api&tab=reviews'); ?>" class="button button-small" style="margin-top: 10px;">
                        <?php _e('Manage Reviews', 'sky-seo-boost'); ?>
                    </a>
                    <?php else : ?>
                    <p class="stat-subtitle"><?php _e('No reviews fetched yet', 'sky-seo-boost'); ?></p>
                    <?php endif; ?>
                </div>
                <?php else : ?>
                <div class="usage-stat">
                    ...
                    <p class="stat-subtitle"><?php _e('No reviews yet', 'sky-seo-boost'); ?></p>
                </div>
                
                <div class="usage-stat">
                    <h4><?php _e('Status', 'sky-seo-boost'); ?></h4>
                    <p class="stat-value" style="font-size: 24px;"><?php _e('Not Configured', 'sky-seo-boost'); ?></p>
                    <p class="stat-subtitle"><?php _e('Add API key to start', 'sky-seo-boost'); ?></p>
                </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    private function render_api_settings($settings) {
        $option_names = $this->get_option_names();
        ?>
        <h2 class="form-section-title"><?php _e('API Configuration', 'sky-seo-boost'); ?></h2>
        <table class="form-table" role="presentation">
            <tr>
                <th scope="row">
                    <label for="serpapi_key"><?php _e('SerpApi Key', 'sky-seo-boost'); ?></label>
                </th>
                <td>
                    ...
                    <p class="description">
                        <?php _e('Get your API key from', 'sky-seo-boost'); ?> 
            ...
            <tr>
                <th scope="row">
                    <label for="place_id"><?php _e('Place ID', 'sky-seo-boost'); ?></label>
                </th>
                <td>
                    ...
                    <p class="description">
                        <?php _e('Enter your Google Place ID or data_id. This uniquely identifies your business on Google.', 'sky-seo-boost'); ?>
                    </p>
                </td>
            </tr>
            ...
            <tr>
                <th scope="row">
                    <label for="business_name"><?php _e('Business Name', 'sky-seo-boost'); ?></label>
                </th>
                <td>
                    <input type="text" 
                           id="business_name"
                           ...
                           class="regular-text"
                           placeholder="<?php esc_attr_e('Your Business Name', 'sky-seo-boost'); ?>" />
                    <p class="description">
                        <?php _e('Enter your business name as it appears on Google.', 'sky-seo-boost'); ?>
                    </p>
                </td>
            </tr>
        </table>
    ...
    private function render_hours_settings($hours_settings) {
        $option_names = $this->get_option_names();
        ?>
        <h2 class="form-section-title"><?php _e('Business Hours', 'sky-seo-boost'); ?></h2>
        <table class="form-table" role="presentation">
            <tr>
                <th scope="row">
                    <label for="timezone"><?php _e('Timezone', 'sky-seo-boost'); ?></label>
                </th>
                <td>
                    ...
                    <p class="description">
                        <?php _e('Select your business timezone for accurate open/closed status.', 'sky-seo-boost'); ?>
                    </p>
                </td>
            </tr>
        </table>
        ...
        <div class="sky-hours-container">
            <div class="hours-actions">
                <button type="button" class="button" id="apply-to-all">
                    <?php _e('Apply to all days', 'sky-seo-boost'); ?>
                </button>
                <button type="button" class="button" id="copy-hours">
                    <?php _e('Copy hours from...', 'sky-seo-boost'); ?>
                </button>
            </div>
    ...
    private function render_hours_table($hours_settings) {
        ...
        <table class="sky-hours-table">
            <thead>
                <tr>
                    <th><?php _e('Day', 'sky-seo-boost'); ?></th>
                    <th><?php _e('Status', 'sky-seo-boost'); ?></th>
                    <th><?php _e('Hours', 'sky-seo-boost'); ?></th>
                ...
                <tr class="day-row" data-day="<?php echo $day_key; ?>">
                    ...
                    <td class="day-status">
                        <select class="status-select" name="<?php echo esc_attr($option_names['hours']); ?>[<?php echo $day_key; ?>_status]" data-day="<?php echo $day_key; ?>">
                            <option value="open" <?php selected(!$is_closed && !$is_24hours); ?>><?php _e('Open', 'sky-seo-boost'); ?></option>
                            <option value="closed" <?php selected($is_closed); ?>><?php _e('Closed', 'sky-seo-boost'); ?></option>
                            <option value="24hours" <?php selected($is_24hours); ?>><?php _e('24 hours', 'sky-seo-boost'); ?></option>
                        </select>
                    ...
                    <td class="day-hours">
                        ...
                        <div class="status-message" style="display: <?php echo $is_closed ? 'block' : 'none'; ?>;">
                            <?php echo $is_closed ? __('Closed all day', 'sky-seo-boost') : ''; ?>
                        </div>
                        <div class="status-message-24" style="display: <?php echo $is_24hours ? 'block' : 'none'; ?>;">
                            <?php echo $is_24hours ? __('Open 24 hours', 'sky-seo-boost') : ''; ?>
                        </div>
                    </td>
                    <td class="day-actions">
                        <button type="button" class="button-link add-hours" data-day="<?php echo $day_key; ?>" style="display: <?php echo (!$is_closed && !$is_24hours) ? 'inline-block' : 'none'; ?>;">
                            <?php _e('Add hours', 'sky-seo-boost'); ?>
                        </button>
                    </td>
                </tr>
                <?php endforeach; ?>
...
private function render_action_buttons($review_stats) {
    ...
    private function render_inline_scripts() {
        ...
        function copyPlaceId() {
            ...
            if (input.value) {
                ...
                button.textContent = '<?php echo esc_js(__('Copied!', 'sky-seo-boost')); ?>';
    ...
    private function display_review_notices() {
        if (isset($_GET['message'])) {
            ...
            $messages = [
                'hidden' => __('Review hidden successfully.', 'sky-seo-boost'),
                'shown' => __('Review shown successfully.', 'sky-seo-boost'),
                'deleted' => __('Review deleted successfully.', 'sky-seo-boost'),
                'updated' => __('Review updated successfully.', 'sky-seo-boost'),
                'bulk_updated' => __('Selected reviews updated successfully.', 'sky-seo-boost'),
            ];
            ...
            <form method="get" action="<?php echo admin_url('admin.php'); ?>">
                ...
                <div class="filter-row">
                    <input type="search" 
                           ...
                           placeholder="<?php esc_attr_e('Search reviews...', 'sky-seo-boost'); ?>"
                           class="search-input">
                    
                    <select name="platform" class="filter-select">
                        <option value="all"><?php _e('All Platforms', 'sky-seo-boost'); ?></option>
                    ...
                    <select name="rating" class="filter-select">
                        <option value="0"><?php _e('All Ratings', 'sky-seo-boost'); ?></option>
                        <?php for ($i = 5; $i >= 1; $i--) : ?>
                    ...
                    <select name="visibility" class="filter-select">
                        <option value="all" <?php selected($filter_visibility, 'all'); ?>><?php _e('All Reviews', 'sky-seo-boost'); ?></option>
                        <option value="visible" <?php selected($filter_visibility, 'visible'); ?>><?php _e('Visible Only', 'sky-seo-boost'); ?></option>
                        <option value="hidden" <?php selected($filter_visibility, 'hidden'); ?>><?php _e('Hidden Only', 'sky-seo-boost'); ?></option>
                    </select>
                    
                    <input type="submit" class="button" value="<?php esc_attr_e('Filter', 'sky-seo-boost'); ?>">
                    ...
                    <?php if (!empty($search) || $filter_rating > 0 || $filter_visibility !== 'all' || $filter_platform !== 'all') : ?>
                        <a href="<?php echo admin_url('admin.php?page=sky-seo-business-api&tab=reviews'); ?>" class="button">
                            <?php _e('Clear Filters', 'sky-seo-boost'); ?>
                        </a>
                    <?php endif; ?>
                </div>
            </form>
        ...
        <div class="tablenav <?php echo $position; ?>">
            <div class="alignleft actions bulkactions">
                <label for="bulk-action-selector-<?php echo $position; ?>" class="screen-reader-text">
                    <?php _e('Select bulk action', 'sky-seo-boost'); ?>
                </label>
                <select name="bulk_action" id="bulk-action-selector-<?php echo $position; ?>">
                    <option value=""><?php _e('Bulk Actions', 'sky-seo-boost'); ?></option>
                    <option value="hide"><?php _e('Hide Selected', 'sky-seo-boost'); ?></option>
                    <option value="show"><?php _e('Show Selected', 'sky-seo-boost'); ?></option>
                    <option value="delete"><?php _e('Delete Selected', 'sky-seo-boost'); ?></option>
                </select>
                <input type="submit" class="button action" value="<?php esc_attr_e('Apply', 'sky-seo-boost'); ?>">
            </div>
            <br class="clear">
        </div>
        <?php
            ...
            <tr>
                <td class="manage-column column-cb check-column">
                    <label class="screen-reader-text" for="cb-select-all-1">
                        <?php _e('Select All', 'sky-seo-boost'); ?>
                    </label>
                    <input id="cb-select-all-1" type="checkbox">
                </td>
                <th class="manage-column column-platform" style="width: 50px;"><?php _e('Source', 'sky-seo-boost'); ?></th>
                <th class="manage-column column-author"><?php _e('Author', 'sky-seo-boost'); ?></th>
                <th class="manage-column column-rating"><?php _e('Rating', 'sky-seo-boost'); ?></th>
                <th class="manage-column column-review"><?php _e('Review', 'sky-seo-boost'); ?></th>
                <th class="manage-column column-date"><?php _e('Date', 'sky-seo-boost'); ?></th>
                <th class="manage-column column-status"><?php _e('Status', 'sky-seo-boost'); ?></th>
                <th class="manage-column column-actions"><?php _e('Actions', 'sky-seo-boost'); ?></th>
            </tr>
        </thead>
        <?php
    }
        ...
        <tr class="<?php echo esc_attr($row_class); ?>">
            ...
            <td class="column-author">
                ...
                <?php if (!empty($review['is_manual'])) : ?>
                    <br><small style="color: #666;"><?php _e('(Manual)', 'sky-seo-boost'); ?></small>
                <?php endif; ?>
            </td>
            ...
            <td class="column-status">
                <?php if ($review['is_visible']) : ?>
                    <span class="status-badge status-visible"><?php _e('Visible', 'sky-seo-boost'); ?></span>
                <?php else : ?>
                    <span class="status-badge status-hidden"><?php _e('Hidden', 'sky-seo-boost'); ?></span>
                <?php endif; ?>
            </td>
                ...
                <div class="row-actions">
                    ...
                    <span class="delete">
                        <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=sky-seo-business-api&tab=reviews&action=delete&review=' . $review['id']), 'review_action_' . $review['id']); ?>" 
                           class="delete-link"
                           onclick="return confirm('<?php esc_attr_e('Are you sure you want to delete this review?', 'sky-seo-boost'); ?>');">
                            <?php _e('Delete', 'sky-seo-boost'); ?>
                        </a>
                    </span>
                </div>
            </td>
        ...
        <div class="tablenav bottom">
            <div class="tablenav-pages">
                <span class="displaying-num">
                    <?php printf(_n('%s item', '%s items', $total_items, 'sky-seo-boost'), number_format_i18n($total_items)); ?>
                </span>
    ...
    private function render_review_modals() {
        ...
        <div id="edit-review-modal" class="review-modal" style="display:none;">
            <div class="modal-content">
                <h3><?php _e('Edit Review', 'sky-seo-boost'); ?></h3>
                <form id="edit-review-form" method="post">
                    ...
                    <div class="modal-actions">
                        <button type="submit" class="button button-primary"><?php _e('Save Changes', 'sky-seo-boost'); ?></button>
                        <button type="button" class="button cancel-edit"><?php _e('Cancel', 'sky-seo-boost'); ?></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    private function render_manual_review_modal() {
        ...
        <div id="manual-review-modal" class="review-modal" style="display:none;">
            <div class="modal-content" style="max-width: 700px;">
                <h3 id="manual-review-title"><?php _e('Add Manual Review', 'sky-seo-boost'); ?></h3>
                <form id="manual-review-form" method="post" enctype="multipart/form-data">
                    ...
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="review-platform"><?php _e('Platform', 'sky-seo-boost'); ?></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="review-author-name"><?php _e('Author Name', 'sky-seo-boost'); ?> <span class="required">*</span></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="review-rating"><?php _e('Rating', 'sky-seo-boost'); ?> <span class="required">*</span></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="review-date"><?php _e('Review Date', 'sky-seo-boost'); ?></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="review-author-photo"><?php _e('Author Photo', 'sky-seo-boost'); ?></label>
                            </th>
                            <td>
                                ...
                                <p class="description"><?php _e('Upload a photo for the reviewer (optional)', 'sky-seo-boost'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">
                                <label for="review-text"><?php _e('Review Text', 'sky-seo-boost'); ?></label>
                            </th>
                            <td>
                    ...
                    <div class="modal-actions">
                        <button type="submit" class="button button-primary"><?php _e('Save Review', 'sky-seo-boost'); ?></button>
                        <button type="button" class="button cancel-manual-review"><?php _e('Cancel', 'sky-seo-boost'); ?></button>
                    </div>
                </form>
            </div>
        </div>
        ...
        jQuery(document).ready(function($) {
            ...
            $('.expand-review').on('click', function(e) {
                ...
                if ($preview.hasClass('expanded')) {
                    ...
                    $(this).text('<?php echo esc_js(__('Read more', 'sky-seo-boost')); ?>');
                } else {
                    ...
                    $.post(ajaxurl, {
                        ...
                        if (response.success) {
                            ...
                            $link.text('<?php echo esc_js(__('Read less', 'sky-seo-boost')); ?>');
                        }
                    });
                }
            });
            ...
            $('.edit-review').on('click', function(e) {
                ...
                if (isManual) {
                    ...
                    $('#manual-review-title').text('<?php echo esc_js(__('Edit Manual Review', 'sky-seo-boost')); ?>');
            ...
            $('#add-manual-review, #add-first-manual-review').on('click', function(e) {
                e.preventDefault();
                $('#manual-review-title').text('<?php echo esc_js(__('Add Manual Review', 'sky-seo-boost')); ?>');
            ...
            $('#manual-review-form').on('submit', function(e) {
                ...
                $submitButton.prop('disabled', true).text('<?php echo esc_js(__('Saving...', 'sky-seo-boost')); ?>');
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $submitButton.prop('disabled', false).text('<?php echo esc_js(__('Save Review', 'sky-seo-boost')); ?>');
                            }
                        }
                    });
                } else {
